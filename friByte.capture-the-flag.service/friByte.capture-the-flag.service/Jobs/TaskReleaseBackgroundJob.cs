...
public class TaskReleaseBackgroundJob:IHostedService,IDisposable
{
    ...
    public Task StartAsync(CancellationToken stoppingToken)
    {
        ...
        _timer = new Timer(DoWork, new DateTimeOffset(DateTime.UtcNow), TimeSpan.Zero,
    ...
    {
        if (state is DateTimeOffset lastRelease)
        {
    ...
    private async void CheckTaskReleaseAsync(DateTimeOffset lastReleaseTime, DateTimeOffset timeNow)
    {
        using (var scope = _scopeFactory.CreateScope())
        {
            ...
            var isNewTasks = await _ctx.CtfTasks
                .AnyAsync(a => a.ReleaseDateTime >= lastReleaseTime && a.ReleaseDateTime <= timeNow);
            if (isNewTasks)
            {
